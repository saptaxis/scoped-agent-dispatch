...
- **Claude:** Pre-configured with permissions, settings, and the plugins named in
  `claude.plugins` (defaults: superpowers, commit-commands, pyright-lsp). Note scad no
  longer ships *itself* as a plugin — its skills install via `~/.agents/skills` — but the
  container still seeds third-party plugins through `enabledPlugins`.
